  ...
  if((pszExt = strrchr(szFilePath, '.'))) {
    if (!strcasecmp(pszExt, ".kbd")) {
      ...
      return FMGR_FORMAT_SET;
               ...
               (!strcasecmp(pszExt, ".gz"))) {
      return FMGR_FORMAT_ZIP;
    } else if ((!strcasecmp(pszExt, ".sbt")) || 
               (!strcasecmp(pszExt, ".dsk")) || 
               (!strcasecmp(pszExt, ".sad")) || 
               (!strcasecmp(pszExt, ".mgt")) || 
               (!strcasecmp(pszExt, ".td0")) ) {
...
psp_display_screen_fmgr(void)
{
  ...
  psp_sdl_back3_print( 370, 6, " R: Delete ", PSP_MENU_NOTE_COLOR);

  psp_sdl_back3_print( 30, 254, " []: Cancel  O/X: Valid  Triangle: Up ", 
...
psp_fmgr_ask_confirm(void)
{
  ...
  psp_sdl_back3_print(290,  70, "Delete a file", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_back3_print(270,  80, "press X to confirm !", 
                     PSP_MENU_WARNING_COLOR);
  psp_sdl_flip();
...
static  int sel=0;
  ...
  for(;;) 
  {
    ...
    if (display_screen) {
      ...
      psp_sdl_back3_rectangle(x, y, 290, rows * 10);
      //display_screen = 0;
    }
    ...
    for(i=0; i<rows; i++){
      ...
      psp_sdl_back3_print(x, y, buffer, color);
      y += 10;
    }
...
int 
psp_fmgr_menu(int format, int disk_id)
{
  static int  first = 1;
  ...
  if (psp_file_request(user_filename, user_filedir)) {
    ...
    else 
    {
      ...
      if (file_format == FMGR_FORMAT_ZIP) {
        if (user_file_format == FMGR_FORMAT_DISK) /* Disk */ error = sim_disk_load(user_filename, disk_id, 1);
      }
      else 
      {
        if (file_format == FMGR_FORMAT_DISK) /* Disk */ error = sim_disk_load(user_filename, disk_id, 0);
        else
        if (file_format == FMGR_FORMAT_KBD) /* Kbd */  error = psp_kbd_load_mapping(user_filename);
        else
        if (file_format == FMGR_FORMAT_SET) /* Settings */  error = sim_load_file_settings(user_filename);
      }
    }
