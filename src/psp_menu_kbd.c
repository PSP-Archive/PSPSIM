...
psp_display_screen_kbd_menu(void)
{
  ...
  //if (psp_menu_dirty) 
  {
    ...
    psp_sdl_back3_print( 30, 6, " Start+L+R: EXIT ", PSP_MENU_WARNING_COLOR);

    psp_display_screen_menu_battery3();

    psp_sdl_back3_print( 310, 6, " Start: Keyboard ", PSP_MENU_NOTE_COLOR);

    psp_sdl_back3_print(30, 254, " []: Cancel  O/X: Valid  Select: Back ", 
                       PSP_MENU_BORDER_COLOR);
    psp_sdl_back3_print(370, 254, " By Zx-81 ",
  ...
  for (menu_id = 0; menu_id < MAX_MENU_KBD_ITEM; menu_id++) 
  {
    ...
    psp_sdl_back3_print(x, y, menu_list[menu_id].title, color);

    if (menu_id == MENU_KBD_SKIN) {
      snprintf(buffer, 30, psp_kbd_skin_dir[psp_kbd_skin]);
      scan = strchr(buffer, '/');
      if (scan) *scan = 0;
      psp_sdl_back3_print(110, y, buffer, color);
    } else
        ...
        (menu_id <= MENU_KBD_JOY_RIGHT)) 
    {
      ...
      if (menu_kbd_selected == KBD_NORMAL_MAPPING  ) sim_key = loc_kbd_mapping[kbd_id];
      else
      if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) sim_key = loc_kbd_mapping_L[kbd_id];
      else
      if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) sim_key = loc_kbd_mapping_R[kbd_id];

      if ((sim_key >= 0) && (sim_key < SIM_MAX_KEY)) {
        strcpy(buffer, psp_sim_key_to_sam_key[sim_key].name);
      } else 
      if (sim_key == KBD_UNASSIGNED) {
        sprintf(buffer, "UNASSIGNED");
      } else
      if (sim_key == KBD_LTRIGGER_MAPPING) {
        sprintf(buffer, "L MAPPING");
      } else
      if (sim_key == KBD_RTRIGGER_MAPPING) {
        sprintf(buffer, "R MAPPING");
      } else {
        sprintf(buffer, "KEY %d", sim_key);
      }
...
psp_keyboard_menu_reset_kbd(void)
{
  ...
  psp_sdl_back3_print(270, 80, "Reset Keyboard !", 
...
{
  psp_display_screen_kbd_menu();
  psp_sdl_back3_print(270, 80, "Set Hotkeys !", 
                     PSP_MENU_WARNING_COLOR);
  psp_menu_dirty = 1;
  psp_sdl_flip();
...
psp_keyboard_menu_load()
{
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back3_print(270, 80, "File loaded !", 
                       PSP_MENU_NOTE_COLOR);
    psp_menu_dirty = 1;
    psp_sdl_flip();
    sleep(1);
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back3_print(270, 80, "Can't load file !", 
                       PSP_MENU_WARNING_COLOR);
    psp_menu_dirty = 1;
    psp_sdl_flip();
    sleep(1);
...
psp_keyboard_menu_mapping(int kbd_id, int step)
{
  ...
  if (ploc_kbd_mapping[kbd_id] <  -3) ploc_kbd_mapping[kbd_id] = SIM_MAX_KEY-1;
  else
  if (ploc_kbd_mapping[kbd_id] >= SIM_MAX_KEY) ploc_kbd_mapping[kbd_id] = -3;
}
...
psp_keyboard_menu_save()
{
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back3_print(270, 80, "File saved !", 
                       PSP_MENU_NOTE_COLOR);
    psp_menu_dirty = 1;
    psp_sdl_flip();
    sleep(1);
  ...
  {
    psp_display_screen_kbd_menu();
    psp_sdl_back3_print(270, 80, "Can't save file !", 
                       PSP_MENU_WARNING_COLOR);
    psp_menu_dirty = 1;
    psp_sdl_flip();
    sleep(1);
    ...
    if (psp_irkeyb != PSP_IRKEYB_EMPTY) {
      ...
      if (sim_key != -1) {
            ...
            (cur_menu_id <= MENU_KBD_JOY_RIGHT)) 
        {
          ...
          if (menu_kbd_selected == -1) loc_kbd_mapping[kbd_id] = sim_key;
          else
          if (menu_kbd_selected == KBD_LTRIGGER_MAPPING) loc_kbd_mapping_L[kbd_id] = sim_key;
          else
          if (menu_kbd_selected == KBD_RTRIGGER_MAPPING) loc_kbd_mapping_R[kbd_id] = sim_key;
        }
      }
    }
# endif
