...
# define MENU_SET_RENDER_X      5
# define MENU_SET_RENDER_Y      6
# define MENU_SET_ANALOG        7
...
# define MAX_MENU_SET_ITEM (MENU_SET_BACK + 1)
  ...
  static int sim_render_x          = 0;
  static int sim_render_y          = 0;
...
psp_display_screen_settings_menu(void)
{
  ...
  for (menu_id = 0; menu_id < MAX_MENU_SET_ITEM; menu_id++) {
    ...
    if (menu_id == MENU_SET_RENDER) {
      ...
      if (sim_render_mode == SIM_RENDER_NORMAL   ) strcpy(buffer, "normal");
      else
      if (sim_render_mode == SIM_RENDER_FIT_WIDTH) strcpy(buffer, "fit width");
      else
      if (sim_render_mode == SIM_RENDER_FIT_HEIGHT) strcpy(buffer, "fit height");
      else
      if (sim_render_mode == SIM_RENDER_FIT      ) strcpy(buffer, "fit");
      else
      if (sim_render_mode == SIM_RENDER_X125     ) strcpy(buffer, "x1.25");
      else
      if (sim_render_mode == SIM_RENDER_MAX_WIDTH) strcpy(buffer, "max width");
      else
      if (sim_render_mode == SIM_RENDER_MAX_HEIGHT) strcpy(buffer, "max height");
      else
      if (sim_render_mode == SIM_RENDER_MAX) strcpy(buffer, "max");

      string_fill_with_space(buffer, 13);
      psp_sdl_back2_print(190, y, buffer, color);
    } else
    if (menu_id == MENU_SET_RENDER_X) {
      sprintf(buffer,"%d", sim_render_x);
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(190, y, buffer, color);
    } else
    if (menu_id == MENU_SET_RENDER_Y) {
      sprintf(buffer,"%d", sim_render_y);
      string_fill_with_space(buffer, 4);
      psp_sdl_back2_print(190, y, buffer, color);
    } else
...
void
psp_settings_menu_render_x(int step)
{
  if (step > 0) {
    if (sim_render_x < 20) sim_render_x++;
    else                         sim_render_x = -20;
  } else {
    if (sim_render_x > -20) sim_render_x--;
    else                          sim_render_x = 20;
  }
...
void
psp_settings_menu_render_y(int step)
{
  if (step > 0) {
    if (sim_render_y < 20) sim_render_y++;
    else                         sim_render_y = -20;
  } else {
    if (sim_render_y > -20) sim_render_y--;
    else                          sim_render_y = 20;
  }
...
psp_settings_menu_init(void)
{
  ...
  sim_render_x       = sim_get_render_x();
  sim_render_y       = sim_get_render_y();
  ...
  psp_reverse_analog = sim_get_psp_reverse_analog();
  psp_display_lr     = sim_get_display_lr();
}
...
psp_settings_menu_validate(void)
{
  ...
  sim_set_render_x           ( sim_render_x );
  sim_set_render_y           ( sim_render_y );
  sim_set_psp_cpu_clock      ( psp_cpu_clock );
  sim_set_speed_limiter      ( sim_speed_limiter );
  sim_set_psp_reverse_analog ( psp_reverse_analog );
  sim_set_frame_skip         ( sim_skip_fps );
  sim_set_view_fps           ( sim_view_fps );
  sim_set_display_lr         ( psp_display_lr );
...
# endif
        ...
        (new_pad & PSP_CTRL_LEFT  )) 
    {
      ...
      switch (cur_menu_id ) 
      {
        ...
        break;              
        case MENU_SET_RENDER_X   : psp_settings_menu_render_x( step );
        break;              
        case MENU_SET_RENDER_Y   : psp_settings_menu_render_y( step );
        break;              
